...
import { FaLinkedin, FaGithub, FaTwitter, FaEnvelope, FaHeart, FaCode, FaEye } from "react-icons/fa";
  ...
  useEffect(() => {
    const updateCounter = async () => {
      try {
        ...
        const scriptUrl = 'https://script.google.com/macros/s/AKfycby_5m0S2A_WXmXv5q9dcfRH-toGTA5bgdl5cJiyhyUQ_Rsprh06Nl-EAeVXm_yDCe8_uw/exec';

        // Send unique visitor ID to count only once per visitor
        const response = await fetch(`${scriptUrl}?visitorId=${visitorId}`);
        ...
        // Fallback - show current count without incrementing
        const currentCount = localStorage.getItem('globalVisitCount');
        setVisitCount(currentCount ? parseInt(currentCount) : 175);
      } finally {
        setLoading(false);
      }
    };
    ...
    <footer className="bg-gray-900 text-white border-t border-gray-700/50 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
          ...
          <div className="flex items-center text-gray-300 text-sm">
            <FaEye className="w-4 h-4 mr-2 text-green-400" />
            <span>{loading ? 'Loading...' : `Visitors: ${visitCount}`}</span>
          </div>
